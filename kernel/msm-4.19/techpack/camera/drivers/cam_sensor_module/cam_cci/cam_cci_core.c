	...
	enum cci_i2c_queue_t queue)
{
	...
	if ((read_val + len + 1) >
		...
		uint32_t report_val = CCI_I2C_REPORT_CMD | (1 << 8) |
			(1 << 9) | (report_id << 4);
		CAM_DBG(CAM_CCI, "CCI_I2C_REPORT_CMD report_id%d cci%dm%dq%d",
			report_id, cci_dev->soc_info.index, master, queue);
		...
		CAM_DBG(CAM_CCI,
			"CCI_I2C_M0_Q0_EXEC_WORD_CNT_ADDR %d, queue: %d cci%dm%d",
			read_val, queue, cci_dev->soc_info.index, master);
		...
		CAM_DBG(CAM_CCI, "reg_offset 0x%x 0x%x cci%dm%dq%d",
			reg_offset, readreg_val, cci_dev->soc_info.index, master, queue);
			queue*0x100 + 3 * 4;
		readreg_val = cam_io_r_mb(base + reg_offset);
		CAM_DBG(CAM_CCI, "reg_offset 0x%x 0x%x cci%dm%dq%d",
			reg_offset, readreg_val, cci_dev->soc_info.index, master, queue);
		...
		CAM_DBG(CAM_CCI, "CCI_QUEUE_START_ADDR cci%dm%dq%d",
...
void cam_cci_wait_reg_prints(struct cci_device *cci_dev,
	enum cci_i2c_master_t master,
	enum cci_i2c_queue_t queue)
{
	uint32_t readreg_val = 0;
	...
	uint32_t readreg_val2 = 0;
	uint32_t reg_offset2 = 0;
	uint32_t readreg_val3 = 0;
	uint32_t reg_offset3 = 0;
	uint32_t readreg_val4 = 0;
	uint32_t reg_offset4 = 0;
	struct cam_hw_soc_info *soc_info =
		&cci_dev->soc_info;
	void __iomem *base = soc_info->reg_map[0].mem_base;

	CAM_DBG(CAM_CCI, "cci%dm%dq%d", cci_dev->soc_info.index, master, queue);
	reg_offset = DEBUG_MASTER_QUEUE_REG_START +  master*0x200 +
		queue*0x100;
	readreg_val = cam_io_r_mb(base + reg_offset);
	CAM_INFO(CAM_CCI, "reg_offset 0x%x 0x%x cci%dm%dq%d",
		reg_offset, readreg_val, cci_dev->soc_info.index, master, queue);
	reg_offset1 = DEBUG_MASTER_QUEUE_REG_START +  master*0x200 +
		queue*0x100 + 3 * 4;
	readreg_val1 = cam_io_r_mb(base + reg_offset1);
	CAM_INFO(CAM_CCI, "reg_offset 0x%x 0x%x cci%dm%dq%d",
		reg_offset1, readreg_val1, cci_dev->soc_info.index, master, queue);
			CAM_DBG(CAM_CCI, "cci%dm%dq%d", cci_dev->soc_info.index, master, queue);
	reg_offset2 = DEBUG_MASTER_REG_START +  master*0x100 +
		5 * 4;
	readreg_val2 = cam_io_r_mb(base + reg_offset2);
	CAM_INFO(CAM_CCI, "reg_offset 0x%x 0x%x cci%dm%dq%d",
		reg_offset2, readreg_val2, cci_dev->soc_info.index, master, queue);
	reg_offset3 = DEBUG_MASTER_QUEUE_REG_START +  master*0x200 +
		queue*0x100 + 2 * 4;
	readreg_val3 = cam_io_r_mb(base + reg_offset3);
	CAM_INFO(CAM_CCI, "reg_offset 0x%x 0x%x cci%dm%dq%d",
		reg_offset3, readreg_val3, cci_dev->soc_info.index, master, queue);
	reg_offset4 = DEBUG_MASTER_QUEUE_REG_START +  master*0x200 +
		queue*0x100 + 4;
	readreg_val4 = cam_io_r_mb(base + reg_offset4);
	CAM_INFO(CAM_CCI, "reg_offset 0x%x 0x%x cci%dm%dq%d",
		reg_offset4, readreg_val4, cci_dev->soc_info.index, master, queue);

}
EXPORT_SYMBOL(cam_cci_wait_reg_prints);
	...
	enum cci_i2c_queue_t queue)
{
	...
	if (!wait_for_completion_timeout(
		...
		for (i = 0; i < 4; i++)
		{
			...
			&cci_dev->cci_master_info[master].report_q[queue],
			CCI_TIMEOUT_1);
			cam_cci_wait_reg_prints(cci_dev, master, queue);
		}
	...
	enum cci_i2c_queue_t queue)
{
	...
	CAM_DBG(CAM_CCI, "CCI_I2C_REPORT_CMD curr_w_cnt: %d reportval=0x%x cci%dm%dq%d rid%d",
		read_val, report_val, cci_dev->soc_info.index, master, queue, report_id);
	cam_io_w_mb(report_val,
		base + CCI_I2C_M0_Q0_LOAD_DATA_ADDR +
		reg_offset);
	read_val++;
	...
	CAM_DBG(CAM_CCI, "CCI_I2C_M0_Q0_EXEC_WORD_CNT_ADDR %d cci%dm%dq%d",
		read_val, cci_dev->soc_info.index, master, queue);
	cam_io_w_mb(read_val, base +
		CCI_I2C_M0_Q0_EXEC_WORD_CNT_ADDR + reg_offset);
}
	...
	enum cci_i2c_queue_t queue)
{
	...
	read_val = cam_io_r_mb(base +
		CCI_I2C_M0_Q0_CUR_WORD_CNT_ADDR + reg_offset);
	CAM_DBG(CAM_CCI, "CCI_I2C_M0_Q0_CUR_WORD_CNT_ADDR %d max %d q%dm%d", read_val,
		cci_dev->cci_i2c_queue_info[master][queue].max_queue_size, queue, master);
	...
	enum cci_i2c_queue_t queue)
{
	...
	if (atomic_read(&cci_dev->cci_master_info[master].q_free[queue]) == 1) {
		...
		spin_unlock_irqrestore(
			&cci_dev->cci_master_info[master].lock_q[queue], flags);
		reg_offset = DEBUG_MASTER_REG_START + master*0x100 + 5 * 4;
		read_val = cam_io_r_mb(base + reg_offset);
		CAM_DBG(CAM_CCI, "offset = 0x%x value = 0x%x cci%dm%dq%d",
			reg_offset, read_val, cci_dev->soc_info.index, master, queue);
		reg_offset = DEBUG_MASTER_QUEUE_REG_START +  master*0x200 +
			queue*0x100 + 8;
		read_val = cam_io_r_mb(base + reg_offset);
		CAM_DBG(CAM_CCI, "308 0x%x cci%dm%dq%d",
			read_val, cci_dev->soc_info.index, master, queue);
		reg_offset = DEBUG_MASTER_QUEUE_REG_START +  master*0x200 +
			queue*0x100 + + 3 * 4;
		read_val = cam_io_r_mb(base + reg_offset);
	...
	enum cci_i2c_sync sync_en)
{
	...
	CAM_DBG(CAM_CCI, "CCI_I2C_M0_Q0_LOAD_DATA_ADDR:val 0x%x:0x%x cci%dm%dq%d",
		CCI_I2C_M0_Q0_LOAD_DATA_ADDR +
		reg_offset, val, cci_dev->soc_info.index, master, queue);
	cam_io_w_mb(val, base + CCI_I2C_M0_Q0_LOAD_DATA_ADDR +
		reg_offset);
	...
	while (cmd_size) {
		uint32_t pack = 0;
		CAM_DBG(CAM_CCI, "pack=%d cci%dm%dq%d",
			pack, cci_dev->soc_info.index, master, queue);
		...
		read_val = cam_io_r_mb(base +
			CCI_I2C_M0_Q0_CUR_WORD_CNT_ADDR + reg_offset);
		CAM_DBG(CAM_CCI, "CUR_WORD_CNT_ADDR %d len %d max %d cci%dm%dq%d",
			read_val, len, max_queue_size, cci_dev->soc_info.index, master, queue);
		...
		if ((read_val + len + 1) > queue_size) {
			if ((read_val + len + 1) > max_queue_size) {
				...
				if (rc < 0) {
					CAM_ERR(CAM_CCI,
						"Failed to process full queue rc: %d cci%dm%dq%d",
						rc, cci_dev->soc_info.index, master, queue);
					return rc;
				}
				continue;
			}
		...
		CAM_DBG(CAM_CCI, "cmd_size %d addr 0x%x data 0x%x cci%dm%dq%d",
			cmd_size, i2c_cmd->reg_addr, i2c_cmd->reg_data, cci_dev->soc_info.index, master, queue);
		delay = i2c_cmd->delay;
		i = 0;
	...
	enum cci_i2c_sync sync_en)
{
	...
	master = c_ctrl->cci_info->cci_i2c_master;
	CAM_DBG(CAM_CCI, "set param sid 0x%x retries %d id_map %d cci%dm%dq%d",
		c_ctrl->cci_info->sid, c_ctrl->cci_info->retries,
		c_ctrl->cci_info->id_map, cci_dev->soc_info.index, master, queue);
	...
	struct cam_cci_ctrl *c_ctrl)
{
	...
	switch (c_ctrl->cmd) {
	CAM_DBG(CAM_CCI, "ctrl_cmd = %d cci%dm%d", c_ctrl->cmd, cci_dev->soc_info.index, master);
	...
	struct cam_cci_ctrl *cci_ctrl)
{
	...
	CAM_DBG(CAM_CCI, "master = %d, cmd = %d cci%d", master, cci_ctrl->cmd, cci_dev->soc_info.index);
