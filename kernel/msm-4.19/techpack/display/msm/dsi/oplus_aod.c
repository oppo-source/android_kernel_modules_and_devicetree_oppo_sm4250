...
static inline bool is_oplus_display_aod_mode(void)
{
...
int oplus_panel_get_aod_light_mode(void *buf)
{
	...
	pr_err("oplus_get_aod_light_mode = %d\n", aod_light_mode);

	return 0;
}

static int dsi_panel_aod_on(struct dsi_panel *panel) {
	int rc = 0;
...
static int dsi_panel_aod_off(struct dsi_panel *panel) {
	int rc = 0;
